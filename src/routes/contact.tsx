...
export const Route = createFileRoute("/contact")({
  ...
  head: () => ({ meta: [{ title: "Contact — Visa Guard Africa" }, { name: "description", content: "Connect with our team for support, partnerships and questions about secure visa processing." }] }),
});
    ...
    <PageLayout>
      <section className="bg-hero-cream">
        <div className="mx-auto max-w-7xl px-6 py-16">
          <span className="inline-flex items-center gap-2 rounded-full border border-border bg-background px-3 py-1 text-xs font-semibold text-primary">• Contact Us</span>
          <h1 className="mt-5 text-4xl font-bold md:text-5xl">Need Help or <span className="italic-serif text-primary">Support?</span></h1>
          <p className="mt-4 max-w-2xl text-muted-foreground">
            Connect With Our Team For Platform Support, Partnership Opportunities, Or Questions About Secure Visa Processing And Travel Protection Services.
          </p>
        </div>
      </section>
        ...
        <div className="mx-auto grid max-w-7xl gap-8 px-6 md:grid-cols-[360px_1fr]">
            ...
            <div className="rounded-2xl border border-border bg-background p-6">
              ...
              <div className="mt-3 flex gap-3">
                {[Facebook, Instagram, Share2].map((Icon, i) => (
                  <a key={i} href="#" className="grid h-9 w-9 place-items-center rounded-full bg-foreground text-background"><Icon className="h-4 w-4" /></a>
                ))}
              </div>
            </div>
          </div>
          ...
          >
            ...
            <div className="mt-6 grid gap-5 sm:grid-cols-2">
              <Field label="Full Name"><input required placeholder="e.g. Alex Chen" className="input" /></Field>
              <Field label="Company"><input placeholder="Company name (optional)" className="input" /></Field>
            </div>
            <Field label="Email Address" className="mt-5"><input type="email" required placeholder="alex@company.com" className="input" /></Field>
            <Field label="Subject of Inquiry" className="mt-5">
              <select className="input"><option>Subject</option><option>Platform Support</option><option>Partnership</option><option>General</option></select>
            </Field>
            <Field label="Your Message" className="mt-5">
              <textarea required rows={5} placeholder="How can we help you?" className="input resize-none" />
            </Field>
            <button className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-primary py-3.5 text-sm font-semibold text-primary-foreground hover:bg-primary/90 sm:w-auto sm:px-10">
              {sent ? "Sent ✓" : <>Send Now <Send className="h-4 w-4" /></>}
            </button>
          </form>
        </div>
      </section>
...
function Field({ label, children, className = "" }: { label: string; children: React.ReactNode; className?: string }) {
  return (
    <label className={`block ${className}`}>
      <span className="mb-2 block text-xs font-semibold uppercase tracking-wider text-foreground/70">{label}</span>
      {children}
    </label>
  );
}
